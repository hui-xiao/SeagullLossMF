...
public class Seagullloss_MF {
	...
	/**
	 * The scheme number of the Seagull-like loss.
	 */
	public static final int Seagull_LOSS_MF = 0;
	 ...
	 * @throws IOException
	 */
	public Seagullloss_MF(RatingSystem paraData) throws IOException {
		wholeD = paraData;
	}// Of SVDExperiment
...
//		initializeSubspaces();
	...
	public static void schemeComparison(RatingSystem paraData, int paraMinimalRounds, int paraNumExperiments) throws IOException {
		...
		for (int j = 0; j < paraNumExperiments; j++) {
				for (int i = L1_LOSS_MF; i < Seagull_LOSS_MF + 1; i++) {
				startTime = System.currentTimeMillis();
				System.out.println("\r\nThe scheme\t" + getNameOfScheme(i) );
				 
				switch (i) {
				case Seagull_LOSS_MF:
					tempMF = new Seagullloss_MF(paraData);
					break;
				default:
...
//				tempMF.setParameters(5, 0.01, 0.05);//the parameters of Seagullloss
				tempMF.setParameters(5, 0.0002, 0.01);//the parameters of L1 loss, L2 loss, Seagullloss
				
				// Step 3. train
				System.out.println("\r\nBeign to train " + getNameOfScheme(i) +" for #" + j + " round(s)");
				tempTrainRoundsMatrix[i][j] = tempMF.train(paraMinimalRounds, 
						paraData.getValiadation(), i, Seagull_LOSS_MF);
